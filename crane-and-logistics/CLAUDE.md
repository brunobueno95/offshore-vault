...
- **Every acronym is expanded at every single use**, not only at first use. If `WLL` appears thirty times, `WLL (Working Load Limit)` appears thirty times. Acronyms are genuinely opaque without expansion and are the highest-value category for every-use repetition. No exceptions.
- **Every Norwegian term is translated at every single use.** Example: `the kranfører (crane operator)` every time. Norwegian terms are opaque to an English-language reader and must carry their English gloss everywhere they appear.
...
- **Common-English role and domain terms do NOT repeat their gloss.** Examples: rigger, banksman, lift supervisor, crane, sling, shackle, hook. Define at first use only if the meaning is not obvious from context; after that the term stands alone for the rest of the article.
